 ...
 * Assistant connection, Standby screen settings, Lighting, Blinds, and
 * per-room active-screen flags (see lib/store.js's defaultProfile doc
 * comment), plus one household-wide Globals record (WiFi + a default
 * Home Assistant connection, see lib/store.js's defaultGlobals doc
 * comment) that a room can opt out of via its own
